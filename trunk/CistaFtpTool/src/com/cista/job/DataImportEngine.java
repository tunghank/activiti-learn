...
public class DataImportEngine {
    ...
    /**
     * Startup all the FTP processors defined.
    ...
    int startup() {
        ...
        for (ImportProcessorConfig p : processors) {
            if (p.isEnable()) {
                logger.info("Startup FTP processor " + p.getName());
                try {
                    ...
                    logger.error("Startup FTP processor " + p.getName()
                ...
                logger.info("FTP processor " + p.getName() + " is not enable, ignored.");
            }
            try {
                sched.start();
            } catch (SchedulerException e) {
    ...
    /**
     * Shutdown all the FTP processors.
    ...
    int shutdown() {
        ...
        for (ImportProcessorConfig p : processors) {
            if (p.isEnable()) {
                logger.info("Shutdown FTP processor " + p.getName());

                try {
                    sched.unscheduleJob(p.getName(), p.getName());
                } catch (SchedulerException e) {
                    logger.error("Shutdown FTP processor " + p.getName()
