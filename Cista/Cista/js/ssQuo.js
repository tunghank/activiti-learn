 ...
 *
 *   like          | SQL Like control , 1 need , 0 not.
*/
SmartSearch.setup = function (params) {
	...
	param_default("width", 680);
	param_default("height", 400);
	param_default("like", 1);
	...
	params.button["onclick"] = function() {
		...
		var target = params.cp + "/SmartSearchPre.action?m=pre";
...
SmartSearchQuo.setup = function (params) {
	function param_default(pname, def) { if (typeof params[pname] == "undefined") { params[pname] = def; }};
	param_default("title", null);
	param_default("button", null);
	param_default("cp", "/");
	...
	params.button["onclick"] = function() {
		...
		var left = (screen.width-width)/2;
		var top = (screen.height-height)/2;
		var target = params.cp + "/SmartSearchQuoPre.action?m=pre";
		target += "&inputField=" + params.inputField;
